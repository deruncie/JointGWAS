collect_results = function(results_list) {
  ...
  results = lapply(traits,function(trait) {
    results_trait = list(
      beta_hats = do.call(dplyr::bind_rows,lapply(results_list,function(x) data.frame(t(x$beta_hats[,trait,drop=FALSE])))),
      SEs = do.call(dplyr::bind_rows,lapply(results_list,function(x) data.frame(t(x$SEs[,trait,drop=FALSE])))),
